...
/**
 * ENKEL ALL-WATERWAYS PROCESSOR
 * 1. Hämta ALLA 142,739 vattendrag
 * 2. Processa alla med smart logik
 * 3. Batch-infoga i water_bodies_unified
 */
...
/**
 * STEG 1: Hämta ALLA vattendrag i batches
 */
async function fetchAllWaterways(): Promise<WaterBody[]> {
    ...
    while (hasMore) {
        console.log(`   📦 Hämtar batch från ${offset.toLocaleString()}...`);
        ...
        if (!data || data.length === 0) {
            ...
            console.log(`   ✅ Hittills hämtat: ${allWaterways.length.toLocaleString()}`);
...
async function processAllWaterways(allWaterways: WaterBody[]): Promise<any[]> {
    ...
    for (const [name, segments] of Object.entries(nameGroups)) {
        ...
        if (nameIndex % 50 === 0) {
            console.log(`   📋 ${nameIndex}/${Object.keys(nameGroups).length}: Processerat ${unifiedWaterways.length} unified waterways...`);
        }
 ...
 * HUVUDPROCESS
 */
async function runCompleteProcess(): Promise<void> {
    console.log('🚀 ENKEL ALL-WATERWAYS PROCESSOR');
    console.log('='.repeat(60));
    
    try {
        // STEG 1: Hämta alla
        const allWaterways = await fetchAllWaterways();
        ...
        if (unifiedWaterways.length < 140000) {
            throw new Error(`För få unified waterways: ${unifiedWaterways.length} (förväntat ~142,739)`);
        }
        ...
        console.log('\n🎉 KOMPLETT PROCESS SLUTFÖRD!');
        console.log('='.repeat(60));
        console.log(`📊 RESULTAT:`);
        console.log(`   Original: ${allWaterways.length.toLocaleString()}`);
        console.log(`   Unified: ${finalCount?.toLocaleString()}`);
        console.log(`   Effektivitet: ${finalCount && finalCount >= 140000 ? '✅ SUCCESS' : '❌ PROBLEM'}`);
        
    } catch (error) {
        console.error('❌ PROCESS MISSLYCKADES:', error);
        throw error;
    }
}
...
if (process.argv[1] === __filename) {
    runCompleteProcess()
        .then(() => {
            console.log('\n✅ Enkel all-waterways processing slutförd!');
            process.exit(0);
        })
        .catch(error => {
            console.error('\n❌ Enkel all-waterways processing misslyckades:', error);
            process.exit(1);
        });
}

export { runCompleteProcess };