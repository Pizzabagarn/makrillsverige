...
# Strömstyrka - Förbättrad logisk färgskala baserad på naturlig progression
REVOLUTIONARY_CURRENT_COLORMAP = [
    [0.000, "#000066"],   # Absolut stillastående - mörk blå
    [0.050, "#000099"],   # Mycket låg ström - blå
    [0.100, "#0033CC"],   # Låg ström - ljusare blå  
    [0.150, "#0066FF"],   # Låg-medium ström - ljus blå
    [0.200, "#3399FF"],   # Medium ström - mycket ljus blå
    [0.250, "#66CCFF"],   # Medium-hög ström - ljusaste blå
    [0.300, "#99DDFF"],   # Hög ström - övergång till cyan
    [0.350, "#CCFFFF"],   # Hög ström - ljus cyan
    [0.400, "#00FFFF"],   # 0.4 m/s - ren cyan
    [0.450, "#00CCCC"],   # Mitt i cyan-zonen - mörkare cyan
    [0.500, "#FFFF00"],   # 0.5 m/s - ren gul
    [0.550, "#FFCC00"],   # Mitt i gul-zonen - orange-gul
    [0.600, "#FF9900"],   # Slutet av gul-zonen - orange
    [0.650, "#FF6600"],   # 0.65 m/s - orange
    [0.700, "#FF4400"],   # Mörkare orange
    [0.750, "#FF2200"],   # Mycket mörkare orange
    [0.800, "#FF0000"],   # Nästan röd
    [0.850, "#DD0000"],   # Början av röd
    [0.900, "#CC0000"],   # 0.9 m/s - röd
    [1.000, "#990000"],   # Mörkare röd
    [1.100, "#770000"],   # Mycket mörkare röd
    [1.200, "#550000"],   # Extremt mörk röd
    [1.300, "#330000"],   # 1.3 m/s - nästan svart röd
    [1.526, "#110000"],   # Absolut maximum - svart-röd
]
...
def improved_traditional_interpolation(lons, lats, values, lon_mesh, lat_mesh, parameter):
    ...
    sigma = smoothing_strength.get(parameter, 1.5)
...
def create_interpolated_image(lons, lats, values, water_mask_grid, output_path, timestamp, bbox, parameter):
    ...
    try:
        ...
        # Säkerställ att imageCoordinates och bbox finns
        if 'imageCoordinates' not in metadata:
            metadata['imageCoordinates'] = {}
