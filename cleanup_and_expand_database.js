#!/usr/bin/env node

// RENSA OCH UTÖKA NORDISK VATTENDRAGS-DATABAS
// 1. Ta bort null-värden och dubletter
// 2. Importera Norge, Danmark och resten av Sverige

import { createClient } from '@supabase/supabase-js';
import { config } from 'dotenv';
import fetch from 'node-fetch';

config({ path: '.env.local' });

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL,
  process.env.SUPABASE_SERVICE_ROLE_KEY
);

// FULLSTÄNDIGA NORDISKA REGIONER
const ALL_NORDIC_REGIONS = [
  // SVERIGE - Kompletta regioner
  { name: 'SE-Stockholm', bbox: '58.8,17.6,59.8,18.8', country: 'Sweden' },
  { name: 'SE-Göteborg', bbox: '57.4,11.7,57.9,12.2', country: 'Sweden' },
  { name: 'SE-Malmö', bbox: '55.4,12.8,55.8,13.3', country: 'Sweden' },
  { name: 'SE-Uppsala', bbox: '59.7,17.4,60.0,17.8', country: 'Sweden' },
  { name: 'SE-Västergötland', bbox: '57.4,11.7,59.1,14.6', country: 'Sweden' },
  { name: 'SE-Östergötland', bbox: '57.7,14.4,59.1,16.9', country: 'Sweden' },
  { name: 'SE-Småland-Väst', bbox: '56.4,13.4,57.3,14.8', country: 'Sweden' },
  { name: 'SE-Småland-Öst', bbox: '56.8,14.7,58.1,16.6', country: 'Sweden' },
  { name: 'SE-Halland', bbox: '56.0,12.0,57.5,13.5', country: 'Sweden' },
  { name: 'SE-Blekinge', bbox: '56.0,14.1,56.8,15.9', country: 'Sweden' },
  { name: 'SE-Dalarna-Syd', bbox: '59.9,13.4,61.2,15.5', country: 'Sweden' },
  { name: 'SE-Dalarna-Nord', bbox: '61.1,13.4,61.9,16.1', country: 'Sweden' },
  { name: 'SE-Gävleborg', bbox: '60.2,15.8,61.8,17.8', country: 'Sweden' },
  { name: 'SE-Värmland', bbox: '58.8,11.9,60.9,14.2', country: 'Sweden' },
  { name: 'SE-Örebro', bbox: '58.9,14.2,59.9,15.8', country: 'Sweden' },
  { name: 'SE-Södermanland', bbox: '58.8,15.8,59.5,17.6', country: 'Sweden' },
  { name: 'SE-Jämtland-Syd', bbox: '62.3,11.9,63.8,15.0', country: 'Sweden' },
  { name: 'SE-Jämtland-Nord', bbox: '63.7,12.0,64.9,16.6', country: 'Sweden' },
  { name: 'SE-Västernorrland', bbox: '62.0,15.8,63.4,18.8', country: 'Sweden' },
  { name: 'SE-Västerbotten-Syd', bbox: '63.4,16.9,65.2,20.5', country: 'Sweden' },
  { name: 'SE-Västerbotten-Nord', bbox: '65.1,17.0,66.1,21.6', country: 'Sweden' },
  { name: 'SE-Norrbotten-Syd', bbox: '64.9,19.9,67.0,23.0', country: 'Sweden' },
  { name: 'SE-Norrbotten-Nord', bbox: '66.9,20.0,69.2,24.3', country: 'Sweden' },
  
  // NORGE - Kompletta regioner
  { name: 'NO-Oslo-Akershus', bbox: '59.3,10.0,60.7,11.8', country: 'Norway' },
  { name: 'NO-Østfold', bbox: '59.0,10.5,59.7,12.0', country: 'Norway' },
  { name: 'NO-Hedmark-Syd', bbox: '60.0,10.5,61.5,12.9', country: 'Norway' },
  { name: 'NO-Hedmark-Nord', bbox: '61.4,10.8,62.8,12.5', country: 'Norway' },
  { name: 'NO-Oppland-Syd', bbox: '60.7,8.7,61.8,10.5', country: 'Norway' },
  { name: 'NO-Oppland-Nord', bbox: '61.7,8.0,62.6,10.2', country: 'Norway' },
  { name: 'NO-Buskerud', bbox: '59.3,8.0,61.0,10.8', country: 'Norway' },
  { name: 'NO-Vestfold', bbox: '58.8,9.8,59.8,10.6', country: 'Norway' },
  { name: 'NO-Telemark', bbox: '58.9,7.9,59.9,9.8', country: 'Norway' },
  { name: 'NO-Aust-Agder', bbox: '58.0,7.8,59.4,9.2', country: 'Norway' },
  { name: 'NO-Vest-Agder', bbox: '57.8,6.5,59.0,8.5', country: 'Norway' },
  { name: 'NO-Rogaland', bbox: '58.0,5.2,59.9,7.2', country: 'Norway' },
  { name: 'NO-Hordaland', bbox: '59.3,4.9,61.2,7.8', country: 'Norway' },
  { name: 'NO-Sogn-Fjordane', bbox: '60.5,4.6,62.2,8.2', country: 'Norway' },
  { name: 'NO-Møre-Romsdal', bbox: '61.7,5.0,63.4,9.2', country: 'Norway' },
  { name: 'NO-Trøndelag-Syd', bbox: '62.4,9.9,63.8,12.9', country: 'Norway' },
  { name: 'NO-Trøndelag-Nord', bbox: '63.7,10.9,64.9,14.2', country: 'Norway' },
  { name: 'NO-Nordland-Syd-A', bbox: '64.9,11.9,66.2,14.5', country: 'Norway' },
  { name: 'NO-Nordland-Syd-B', bbox: '66.1,13.9,67.6,16.1', country: 'Norway' },
  { name: 'NO-Nordland-Nord-A', bbox: '67.5,13.9,68.5,16.8', country: 'Norway' },
  { name: 'NO-Nordland-Nord-B', bbox: '68.4,15.5,69.6,18.6', country: 'Norway' },
  { name: 'NO-Troms', bbox: '68.4,16.2,70.3,21.2', country: 'Norway' },
  { name: 'NO-Finnmark-Vest', bbox: '70.0,20.0,71.2,26.0', country: 'Norway' },
  { name: 'NO-Finnmark-Öst', bbox: '69.8,26.0,71.2,31.0', country: 'Norway' },
  
  // DANMARK - Kompletta regioner  
  { name: 'DK-Sjælland', bbox: '55.1,11.0,56.1,12.7', country: 'Denmark' },
  { name: 'DK-Fyn', bbox: '55.0,9.7,55.7,10.9', country: 'Denmark' },
  { name: 'DK-Jylland-Syd', bbox: '54.8,8.0,56.0,10.0', country: 'Denmark' },
  { name: 'DK-Jylland-Vest', bbox: '55.4,7.8,57.8,9.0', country: 'Denmark' },
  { name: 'DK-Jylland-Öst', bbox: '55.8,9.0,57.8,11.2', country: 'Denmark' },
  { name: 'DK-Jylland-Nord', bbox: '56.4,8.1,57.8,10.7', country: 'Denmark' },
  { name: 'DK-Bornholm', bbox: '55.0,14.6,55.4,15.2', country: 'Denmark' }
];

// RENSA DATABAS FÖRST
async function cleanupDatabase() {
  console.log('🧹 RENSAR DATABAS FRÅN NULL-VÄRDEN OCH DUBLETTER...\n');
  
  // 1. Ta bort poster med null-name OCH null-geometri
  console.log('🗑️ Tar bort poster utan namn OCH geometri...');
  const { count: deletedNulls } = await supabase
    .from('water_bodies')
    .delete()
    .is('name', null)
    .is('geometry', null);
  
  console.log(`✅ Tog bort ${deletedNulls || 0} helt tomma poster`);
  
  // 2. Ta bort poster med ogiltiga geometrier
  console.log('🗑️ Tar bort poster med ogiltiga geometrier...');
  const { count: deletedInvalid } = await supabase
    .from('water_bodies')
    .delete()
    .or('geometry.is.null,water_type.is.null,osm_id.is.null');
  
  console.log(`✅ Tog bort ${deletedInvalid || 0} ogiltiga poster`);
  
  // 3. Enkel dublett-rensning via osm_id check
  console.log('🔍 Rensar potentiella dubletter...');
  
  // Hämta alla osm_id som finns mer än en gång
  const { data: allWaters } = await supabase
    .from('water_bodies')
    .select('id, osm_id, osm_type')
    .not('osm_id', 'is', null)
    .order('osm_id, id');
  
  if (allWaters && allWaters.length > 0) {
    const seenIds = new Set();
    const duplicateIds = [];
    
    for (const water of allWaters) {
      const key = `${water.osm_id}-${water.osm_type}`;
      if (seenIds.has(key)) {
        duplicateIds.push(water.id);
      } else {
        seenIds.add(key);
      }
    }
    
    if (duplicateIds.length > 0) {
      console.log(`🗑️ Hittade ${duplicateIds.length} dubletter, tar bort...`);
      
      // Ta bort i mindre batchar
      const BATCH_SIZE = 100;
      for (let i = 0; i < duplicateIds.length; i += BATCH_SIZE) {
        const batch = duplicateIds.slice(i, i + BATCH_SIZE);
        await supabase
          .from('water_bodies')
          .delete()
          .in('id', batch);
      }
      console.log(`✅ Tog bort ${duplicateIds.length} dubletter`);
    } else {
      console.log(`✅ Inga dubletter hittade`);
    }
  }
  
  // 4. Visa slutstatistik
  const { count: finalCount } = await supabase
    .from('water_bodies')
    .select('*', { count: 'exact', head: true });
    
  const { count: namedCount } = await supabase
    .from('water_bodies')
    .select('*', { count: 'exact', head: true })
    .not('name', 'is', null);
  
  console.log(`\n📊 EFTER RENSNING:`);
  console.log(`   💧 ${finalCount} totala vatten`);
  console.log(`   🏷️ ${namedCount} namngivna vatten`);
  console.log(`   📈 ${Math.round((namedCount/finalCount)*100)}% har namn\n`);
}

// UTÖKAD OVERPASS QUERY
function createNordicQuery(bbox) {
  return `
[out:json][timeout:200];
(
  // WAYS - Mindre sjöar och årar
  way["natural"="water"](${bbox});
  way["water"="lake"](${bbox});
  way["waterway"="river"](${bbox});
  way["landuse"="reservoir"](${bbox});
  way["leisure"="fishing"](${bbox});
  
  // RELATIONS - STORA SJÖAR OCH KOMPLEXA VATTEN
  relation["natural"="water"](${bbox});
  relation["water"="lake"](${bbox});
  relation["place"="lake"](${bbox});
  relation["landuse"="reservoir"](${bbox});
  relation["leisure"="fishing"](${bbox});
);
out geom;
`;
}

// ROBUST DATA FETCH MED RETRY
async function fetchNordicWaterData(region) {
  const query = createNordicQuery(region.bbox);
  const maxRetries = 3;
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`🔄 ${region.name}: Försök ${attempt}/${maxRetries}...`);
      
      const response = await fetch('https://overpass-api.de/api/interpreter', {
        method: 'POST',
        body: query.trim(),
        headers: { 'Content-Type': 'text/plain; charset=utf-8' },
        signal: AbortSignal.timeout(300000) // 5 min
      });

      if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
      }

      const data = await response.json();
      console.log(`✅ ${region.name}: ${data.elements?.length || 0} element`);
      
      const ways = data.elements.filter(e => e.type === 'way').length;
      const relations = data.elements.filter(e => e.type === 'relation').length;
      console.log(`   📊 ${ways} ways, ${relations} relations`);
      
      return data;
      
    } catch (error) {
      console.error(`❌ ${region.name} försök ${attempt}: ${error.message}`);
      if (attempt === maxRetries) throw error;
      
      // Vänta innan retry
      await new Promise(resolve => setTimeout(resolve, 5000));
    }
  }
}

// AVANCERAD PROCESSNING
function processNordicWaterData(osmData, region) {
  const processed = [];
  
  for (const element of osmData.elements) {
    if (!element.tags || !element.geometry) continue;
    
    const name = element.tags.name || 
                 element.tags['name:sv'] || 
                 element.tags['name:no'] || 
                 element.tags['name:da'] || null;
    
    let water_type = 'water';
    
    // Smart typbestämning
    if (element.tags.water === 'lake' || element.tags.place === 'lake') {
      water_type = 'lake';
    } else if (element.tags.waterway === 'river') {
      water_type = 'river';
    } else if (element.tags.waterway === 'stream') {
      water_type = 'stream';
    } else if (element.tags.landuse === 'reservoir') {
      water_type = 'reservoir';
    } else if (element.tags.leisure === 'fishing') {
      water_type = 'fishing';
    }
    
    // Filtrera bort diken och avlopp
    if (element.tags.water === 'ditch' || 
        element.tags.water === 'drain' ||
        element.tags.waterway === 'ditch' ||
        element.tags.waterway === 'drain') {
      continue;
    }
    
    // Hantera geometri
    let geometry = null;
    
    if (element.type === 'way' && element.geometry.length >= 2) {
      const coords = element.geometry.map(node => [node.lon, node.lat]);
      
      const isPolygon = coords.length > 2 && 
                       coords[0][0] === coords[coords.length-1][0] && 
                       coords[0][1] === coords[coords.length-1][1];
      
      const wktCoords = coords.map(coord => `${coord[0]} ${coord[1]}`).join(',');
      
      if (isPolygon) {
        geometry = `SRID=4326;POLYGON((${wktCoords}))`;
      } else {
        geometry = `SRID=4326;LINESTRING(${wktCoords})`;
      }
      
    } else if (element.type === 'relation' && element.members) {
      const outerWays = element.members.filter(m => 
        (m.role === 'outer' || m.role === '') && 
        m.geometry && 
        m.geometry.length > 2
      );
      
      if (outerWays.length > 0) {
        const coords = outerWays[0].geometry.map(node => [node.lon, node.lat]);
        
        if (coords.length >= 3) {
          if (coords[0][0] !== coords[coords.length-1][0] || 
              coords[0][1] !== coords[coords.length-1][1]) {
            coords.push(coords[0]);
          }
          
          const wktCoords = coords.map(coord => `${coord[0]} ${coord[1]}`).join(',');
          geometry = `SRID=4326;POLYGON((${wktCoords}))`;
          
          if (name) {
            console.log(`  🏞️ ${name} (${water_type}, relation)`);
          }
        }
      }
    }

    // ENDAST SPARA MED GILTIG GEOMETRI OCH DATA
    if (geometry && element.id && element.type && water_type) {
      processed.push({
        osm_id: element.id,
        osm_type: element.type,
        name: name, // Kan vara null, det är OK
        water_type: water_type,
        geometry: geometry,
        tags: element.tags || {}
      });
    }
  }
  
  const lakes = processed.filter(w => w.water_type === 'lake').length;
  const rivers = processed.filter(w => w.water_type === 'river').length;
  const named = processed.filter(w => w.name).length;
  
  console.log(`🎣 ${region.name}: ${processed.length} vatten (${named} namngivna)`);
  console.log(`   📊 Sjöar: ${lakes}, Årar: ${rivers}`);
  
  return processed;
}

// SÄKER DATABAS-SPARNING
async function saveNordicWaterData(waters, region) {
  if (waters.length === 0) return 0;
  
  console.log(`💾 ${region.name}: Sparar ${waters.length} vatten...`);
  
  const BATCH_SIZE = 100;
  let saved = 0;
  let errors = 0;
  
  for (let i = 0; i < waters.length; i += BATCH_SIZE) {
    const batch = waters.slice(i, i + BATCH_SIZE);
    
    try {
      // Först kontrollera om de redan finns
      const existingIds = await Promise.all(
        batch.map(async (water) => {
          const { count } = await supabase
            .from('water_bodies')
            .select('*', { count: 'exact', head: true })
            .eq('osm_id', water.osm_id)
            .eq('osm_type', water.osm_type);
          return count > 0 ? water.osm_id : null;
        })
      );
      
      // Filtrera bort redan existerande
      const newWaters = batch.filter((water, idx) => !existingIds[idx]);
      
      if (newWaters.length === 0) continue;
      
      const { data, error } = await supabase
        .from('water_bodies')
        .insert(newWaters)
        .select('id');
      
      if (error) {
        console.error(`❌ Batch ${i}: ${error.message}`);
        errors++;
        continue;
      }
      
      saved += data?.length || 0;
      
      if (saved % 500 === 0 || saved === waters.length) {
        console.log(`✅ ${region.name}: ${saved}/${waters.length} sparade`);
      }
      
    } catch (error) {
      console.error(`💥 Batch ${i}: ${error.message}`);
      errors++;
    }
    
    await new Promise(resolve => setTimeout(resolve, 200));
  }
  
  if (errors > 0) {
    console.log(`⚠️ ${region.name}: ${errors} batchar misslyckades`);
  }
  
  return saved;
}

// HUVUDFUNKTION
async function runCompleteNordicImport() {
  console.log('🌊 KOMPLETT NORDISK VATTENDRAGS-DATABAS');
  console.log('🧹 Rensar först, importerar sedan hela Norden\n');
  
  // Steg 1: Rensa databas
  await cleanupDatabase();
  
  // Steg 2: Importera alla regioner
  console.log(`🚀 STARTAR IMPORT AV ${ALL_NORDIC_REGIONS.length} REGIONER\n`);
  
  let totalSaved = 0;
  let successful = 0;
  const results = [];
  
  for (const region of ALL_NORDIC_REGIONS) {
    try {
      console.log(`\n🗺️ ${region.name} (${region.country})`);
      
      const osmData = await fetchNordicWaterData(region);
      const waters = processNordicWaterData(osmData, region);
      const saved = await saveNordicWaterData(waters, region);
      
      totalSaved += saved;
      successful++;
      results.push({ region: region.name, country: region.country, saved });
      
      console.log(`✅ ${region.name}: ${saved} nya vatten sparade`);
      
      // Paus mellan regioner för att inte överbelasta
      await new Promise(resolve => setTimeout(resolve, 3000));
      
    } catch (error) {
      console.error(`❌ ${region.name} MISSLYCKADES: ${error.message}`);
      results.push({ region: region.name, country: region.country, saved: 0, error: error.message });
      continue;
    }
  }
  
  // SLUTRAPPORT
  console.log('\n🎯 SLUTGILTIG RAPPORT:');
  console.log(`✅ Framgångsrika regioner: ${successful}/${ALL_NORDIC_REGIONS.length}`);
  console.log(`💾 Nya vatten sparade: ${totalSaved}`);
  
  // Visa per land
  const byCountry = results.reduce((acc, r) => {
    acc[r.country] = (acc[r.country] || 0) + r.saved;
    return acc;
  }, {});
  
  console.log('\n📊 PER LAND:');
  Object.entries(byCountry).forEach(([country, count]) => {
    console.log(`   ${country}: ${count} vatten`);
  });
  
  // Final statistik
  const { count: finalTotal } = await supabase
    .from('water_bodies')
    .select('*', { count: 'exact', head: true });
  
  console.log(`\n🏆 TOTAL DATABAS: ${finalTotal} nordiska vatten`);
  console.log('🎉 KOMPLETT NORDISK VATTENDRAGS-DATABAS KLAR!');
}

// START
runCompleteNordicImport().catch(error => {
  console.error('💥 Nordisk import kraschade:', error);
  process.exit(1);
}); 