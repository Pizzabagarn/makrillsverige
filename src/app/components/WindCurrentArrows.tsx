...
export default function WindCurrentArrows({
  ...
  useEffect(() => {
    ...
    if (!map.hasImage(iconImageId)) {
      // Skapa en enkel pil som SVG data URL
      const arrowSvg = `
        <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 2L12 22M19 9L12 2L5 9" 
                stroke="white" 
                stroke-width="2" 
      ...
      const svgDataUrl = 'data:image/svg+xml;base64,' + btoa(arrowSvg);
      
      // Ladda bilden som SDF för att kunna färga den
      const img = new Image();
      img.onload = () => {
        if (map.hasImage(iconImageId)) return; // Undvik dubbel-adding
        map.addImage(iconImageId, img, { sdf: true });
      };
      img.src = svgDataUrl;
    }
  }, [map, iconImageId]);
      ...
      map.addLayer({
        ...
        layout: {
          ...
          'icon-size': [
            ...
            ['linear'],
            ['get', `${type}Speed`],
            0, 0.3,     // Minsta storlek
            maxSpeed, 1.2  // Största storlek
          ],
