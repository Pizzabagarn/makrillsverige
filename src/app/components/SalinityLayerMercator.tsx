...
const SalinityLayerMercator = React.memo<SalinityLayerMercatorProps>(({ 
  ...
  useEffect(() => {
    const loadMetadata = async () => {
      try {
        const response = await fetch('/data/salinity-images-mercator/metadata.json');
        if (response.ok) {
          const data = await response.json();
          setMetadata(data);
          console.log('✅ Salinity Mercator metadata laddad:', data);
        } else if (response.status === 404) {
          console.warn('⚠️ Salinity Mercator metadata inte funnen - bilder behöver genereras först');
        } else {
          console.error('❌ Kunde inte ladda Salinity Mercator metadata');
        }
      } catch (error) {
        console.warn('⚠️ Salinity Mercator metadata inte tillgänglig:', error instanceof Error ? error.message : String(error));
      }
    };
  ...
  useEffect(() => {
    ...
    const preloadImages = async () => {
      ...
      for (const imageInfo of metadata.images) {
        const img = new Image();
        const imageUrl = `/data/salinity-images-mercator/${imageInfo.filename}`;
  ...
  useEffect(() => {
    ...
    if (nearestImage) {
      const newImageUrl = `/data/salinity-images-mercator/${nearestImage.filename}`;
  ...
  return (
    <Source id="salinity-mercator-source" {...rasterSource}>
      <Layer {...rasterLayer} />
    </Source>
  );
});
