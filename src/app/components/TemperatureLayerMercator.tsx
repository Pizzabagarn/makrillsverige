...
const TemperatureLayerMercator = React.memo<TemperatureLayerMercatorProps>(({ 
  ...
  useEffect(() => {
    ...
    const preloadImages = async () => {
      console.log(`🚀 Bakgrundspreloading av ${metadata.images.length} Temperature Mercator-bilder...`);
      ...
      for (const imageInfo of metadata.images) {
        ...
        img.onload = () => {
          ...
          loadedCount++;
          if (loadedCount % 5 === 0) {
            console.log(`✅ Preloaded ${loadedCount}/${metadata.images.length} Temperature Mercator-bilder`);
          }
        ...
        img.onerror = () => {
          console.log(`⚠️ Kunde inte preload Temperature Mercator-bild: ${imageInfo.filename}`);
        };
      ...
      console.log(`🎉 Alla ${loadedCount} Temperature Mercator-bilder preloadade!`);
    };
    ...
    if (nearestImage) {
      ...
      if (newImageUrl !== currentImageUrl) {
        ...
        if (preloadedImg) {
          ...
          console.log('⚡ Instant Temperature Mercator bild från cache:', nearestImage.filename);
        } else {
          // Bilden är inte preloaded, ladda den direkt
          setImageLoaded(false);
          console.log('⏳ Laddar Temperature Mercator bild (inte cached):', nearestImage.filename);
          ...
          img.onload = () => {
            ...
            if (img.src === newImageUrl) {
              setImageLoaded(true);
              console.log('✅ Temperature Mercator bild laddad:', nearestImage.filename);
            }
          };
          img.onerror = () => {
