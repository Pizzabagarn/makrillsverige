...
export default function LoginPage() {
  const { user, signIn, signInWithGoogle, signInWithMagicLink, loading: authLoading } = useAuth()
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setError('Ett oväntat fel uppstod med Google-inloggning')
    }
  }
  ...
  const handleMagicLink = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setSuccess('Magisk länk skickad! Kolla din e-post.')
      }
    } catch (error) {
      setError('Ett oväntat fel uppstod')
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-300">Logga in på ditt Makrill-Sverige konto</p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full text-blue-400 hover:text-blue-300 font-medium py-2 transition-colors duration-200 flex items-center justify-center space-x-2"
            >
              {magicLinkLoading ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
                <span>Skicka magisk länk istället</span>
              )}
            </button>
          </form>
          ...
          <div className="mt-6 pt-6 border-t border-white/20 text-center space-y-2">
            ...
            <Link href="/auth/forgot-password" className="text-slate-400 hover:text-slate-300 text-sm block">
              Glömt lösenordet?
            </Link>
          </div>
        </div>
